    ...
    >
          ...
          >
            {heading.parts && heading.parts.length > 0 ? (
              heading.parts.map((part, index) => (
                <span
                  key={index}
                  className={part.highlight ? "text-[#FFF200]" : ""}
                >
                  {part.text}
              ...
              <>
                Transform your{" "}
                <span className="text-[#FFF200]">mindset, career,</span> and{" "}
                <span className="text-[#FFF200]">business</span> in{" "}
                <span className="text-[#FFF200]">90 days</span> with my
              ...
              {(() => {
                const guaranteeText =
                  typeof guarantee === "string"
                    ? guarantee
                    : guarantee?.text ||
                      "If you don't get results, I'll work with you again for free until you get results.";
                const parts = guaranteeText.split(/(free)/gi);
                return parts.map((part, i) =>
                  part.toLowerCase() === "free" ? (
                    <span key={i} className="text-[#FFF200] font-semibold">
                      {part}
                    </span>
                  ) : (
      ...
      <ApplyNowModal
        ...
        formConfig={{
          ...form,
          courseName:
            heading?.parts?.map((p) => p.text).join(" ") ||
            "Expertisor Academy Course",
