...
const OfferBanner = ({ offer_detail }) => {
  ...
  useEffect(() => {
    ...
    let expiry = localStorage.getItem("offer_expiry");

    if (!expiry) {
      // If not found, create a new expiry
      expiry = Date.now() + DURATION * 1000;
      localStorage.setItem("offer_expiry", expiry);
    } else {
  ...
  const Actions = () => (
    <div className="flex items-center gap-2">
      <div onClick={() => setShowApplyModal(true)} className="bg-yellow cursor-pointer text-black text-[14px] lg:text-[18px] font-semibold px-4 lg:px-5 py-2 lg:py-3  rounded">
        Join Now
      </div>
