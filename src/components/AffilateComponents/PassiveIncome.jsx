...
import { detectCountryCode, getPhonePrefix, extractCourseFromURL } from '../../utils/geolocation'
...
const PassiveIncome = ({ p_income }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const courseName = extractCourseFromURL() || 'SolidWorks Design Mastery Program';
            ...
            if (text) {
                ...
                if (!result.success) {
                    console.warn('API returned error, but continuing...');
                }
            }
        } catch (error) {
        ...
        setFormData({ name: '', email: '', countryCode: '+91', phone: '', qualification: '', profile: '' });
    };
                        ...
                        <div className='mt-10 lg:mt-20 md:grid grid-cols-2 gap-8 flex flex-col'>
                            ...
                            <div className='flex flex-col gap-6'>
                                ...
                                <div className='flex flex-col gap-3 lg:gap-4 p-6 bg-[#000000BF] border border-[#2D2D2D] rounded-2xl'>
                                    ...
                                    <StartButton data={p_income.start_button} onClick={() => setShowApplyModal(true)} />
                                </div>

                                {/* Pointers below card */}
                                {p_income.what_you_get && (
                                    <div className='p-4 bg-[#1a1a1a] rounded-xl border border-[#2D2D2D]'>
                                        <h5 className='font-inter font-semibold text-[16px] leading-[100%] tracking-[0] text-[#FFFFFF] mb-4'>{p_income.what_you_get?.head}</h5>
                                        <div className='flex flex-col gap-2'>
                                            {p_income.what_you_get?.benefit?.map((data, i) => (
                                                <div key={i} className='flex gap-2 items-center'>
                                                    <img src={p_income.what_you_get?.tick_img} alt="" className='w-5 h-5' />
                                    ...
                                    <div className='w-full max-w-[380px] p-4 sm:p-5 bg-black border border-white/10 rounded-xl'>
                                        ...
                                        <div className="text-[#8A8A8A] mt-1">
                                            ...
                                            <span className="relative inline-flex items-center px-3 py-1">
                                                ...
                                                />
                                                <span className="relative z-10 text-[11px]">1:1 Guidance</span>
                                            </span>
                                        </div>
                            ...
                            {!p_income.show_form && p_income.what_you_get && (
                                <div className='flex flex-col sm:flex-row items-start sm:items-center gap-6'>
                                    ...
                                    {p_income.what_you_get?.img && (
                                        <img src={p_income.what_you_get.img} alt="What you get" className='w-[140px] sm:w-[180px] object-contain' />
                                    )}
                                </div>
                            )}
